...
println!("=========================================");
println!("IDSM Configuration");
println!("IP   : {}", idsm_ip);
println!("Port : {}", idsm_port);
println!("=========================================");
...
// ===============================================
// IDSM Configuration (Command Line Arguments)
// ===============================================



let idsm_port = if args.len() > 2 && !args[2].starts_with("--") {
    args[2].clone()
} else {
    "8081".to_string()
};
                    ...
                    Ok(Some(line)) => {
...

                        {
                            ...
                            let detected_alerts =
                                ...
                                if !detected_alerts.is_empty() {

    println!();
    println!("=================================");
    println!("Detector returned {} alert(s)", detected_alerts.len());
    println!("=================================");
}
...
/*
 * Send to AUTOSAR IDSM
 */
if let Some(mac) = &incident.device_mac {

    println!("About to send incident to IDSM...");

    if let Err(error) = idsm::send_to_idsm(
    &idsm_ip,
    &idsm_port,
    17,
...
let security_event =
    event_generator.generate(&incident);
        ...
        error
            ...
            _ = soc_timer.tick() => {
...

                {
                  ...
                  if let Err(error) = event_sender.update(&incident) {
